...
class CloneLabel(QLabel):
    def __init__(self, pixmap):
        ...
        # Отрисовываем pixmap с учетом начального масштаба
        self.update_pixmap()
    ...
    def update_pixmap(self):
        ...
        # Сбрасываем выделение при изменении масштаба, 
        # чтобы избежать визуальных проблем с несовпадением координат
        self.start_point = QPoint()
        self.end_point = QPoint()
        self.selected_rect = QRect()
        self.is_drawing = False
    ...
    def paintEvent(self, event):
        ...
        # Настройка стиля линии выделения
    ...
    def get_normalized_rect(self):
        ...
        # Возвращаем относительные координаты (от 0 до 1)
        # Поскольку ширина и высота берутся от текущего (масштабированного) pixmap,
        # координаты всегда будут правильными относительно оригинального изображения
...
class ImageCloneDialog(QDialog):
    def __init__(self, pixmap, parent=None):
        ...
        self.setWindowTitle("Клонирование фото")
        self.resize(900, 700)
        self.rect_ratio = None

        layout = QVBoxLayout(self)
        
        inst_label = QLabel("Зажмите левую кнопку мыши и выделите область с фото для клонирования.")
        ...
        # Автоматически подгоняем масштаб, чтобы страница помещалась в окно при открытии
        if not pixmap.isNull():
            w_avail = 850.0  # Доступная ширина окна с учетом отступов
            h_avail = 550.0  # Доступная высота окна с учетом отступов
            scale_w = w_avail / pixmap.width()
            scale_h = h_avail / pixmap.height()
            
            initial_scale = min(scale_w, scale_h)
            # Если лист маленький, не растягиваем его больше 100% (оригинала)
            initial_scale = min(1.0, initial_scale)
            self.clone_label.set_scale(initial_scale)

        btn_layout = QHBoxLayout()
        
        # Кнопки масштабирования + и -
        ...
        self.btn_apply = QPushButton("Клонировать выделенное")
    ...
    def zoom_out(self):
        current_scale = self.clone_label.get_scale()
        # Ограничиваем минимальное отдаление
